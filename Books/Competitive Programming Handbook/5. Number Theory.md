...
this helps calculate the a^n in O(logn) time rather than the naive approach of the O(n)

so given a a^n we can represent the n in 2 base for eg.
3^13 =3^1101 =3^8 . 3^4 . 3^1

hence the a^n = 1 (if n is 0), 
						(a^2)^n/2 (if a is even)
						(a^2)^n-1/2 . a (if a is odd)
```cpp
